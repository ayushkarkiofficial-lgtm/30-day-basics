...
## Day 24 — SEO, Performance & Share Preview (Tasklift app) — 2026-06-15

Scope: the live Tasklift React app only (`tasklift-mvp-app`). Static marketing site
left untouched. Goal: make the public page findable, shareable, and load-clean.

### Built

- `index.html` `<head>`: keyword `<title>`, rewritten meta description, `canonical`,
  `theme-color`, favicons (svg + 32px png + apple-touch — app had none), full
  Open Graph + Twitter Card tags. (These live in static HTML because link crawlers
  don't run JS.)
- `public/`: `og-image.png` (1200×630, branded, generated by `scripts/gen_og_assets.py`
  with Pillow), `favicon.svg`, `favicon-32.png`, `apple-touch-icon.png`, `robots.txt`,
  `sitemap.xml` (single URL — it's an SPA).
- Accessibility fix: `FileUpload.jsx` file `<input>` had no label → added `aria-label`
  (Lighthouse "form elements do not have associated labels"). IntakeForm inputs were
  already wrapped in `<label>`.

### Verified (against the LIVE deployed URL, not localhost)

- All assets serve 200 with correct content-types (curled robots/sitemap/og-image/icons). ✅
- Share preview on opengraph.xyz: 0 errors, 12 checks pass. ✅
- Build clean, 16/16 vitest pass.
- PageSpeed/Lighthouse run; findings logged (below).

### Iterations this session (each = commit + push → Netlify redeploy)

1. `4da307c` core SEO/meta + assets.
2. `0923c9a` added a CTA pill ("Request a workflow review →") to og-image — the one
   advisory opengraph.xyz raised ("no CTA in image").
3. `ac77110` cache-busted og:image URL (`?v=2`) — image was already live but preview
   tools cache by URL and kept showing the old one.
4. `82ac2c6` a11y label fix + logged PageSpeed findings.

### Performance findings — logged, deferred (Day 24 asks to log, not chase)

- Reduce unused JS (~62 KiB) + render-blocking (~80 ms): both = the `@supabase/supabase-js`
  bundle (single 378 kB / 107 kB gzip chunk, no splitting). Real fix = code-split /
  lazy-load. Deferred.
- "Document request latency — Error!": Netlify free-tier TTFB/cold-edge variance, not
  app code. Fluctuates between runs.

### Lessons this session

- **OG/meta tags must be in static HTML.** Link scrapers (LinkedIn, Slack, FB) don't run
  your SPA's JavaScript, so JS-injected tags are invisible. `index.html`, not components.
- **OG image = PNG, not SVG**, at 1200×630. FB/LinkedIn/X won't reliably render SVG cards.
- **Preview tools cache the image by URL.** After updating the image, bump the URL
  (`?v=2`) or use the platform's "scrape again" — otherwise you keep seeing the old one.
- **Always verify against the deployed URL**, never localhost — crawlers fetch over the net.
- **Git 403 "denied to <account>"** = wrong cached GitHub account *or* an unaccepted
  collaborator invite, not a code/harness problem. (Pushing to this repo needs `AiferAifa`
  accepted as collaborator; GCM caches that account.)
- **Windows build EPERM on `dist/`** = a stray `vite preview` node child still holds the
  folder (npm got killed, child orphaned). Kill the process, then rebuild.
